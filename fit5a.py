...
for i in eari:
    print(i)
    l=l+1
    plt.plot(ashp,yres[0:10,:,l].mean(axis=0)*i,label=str(i)+' RI')
    plt.fill_between(ashp,yres1[0:10,:,l].mean(axis=0)*i, yres2[0:10,:,l].mean(axis=0)*i,alpha=.4)
#    plt.plot(ashp,yres[1,:,l]*i,label=str(i)+' RI')
plt.title('Sampling for 10% return value uncertainty')
plt.xlabel('Shape')
plt.ylabel('Number of Samples')
plt.legend()
...
for i in eari:
    ...
    l=l+1
#plt.plot(ashp,(yres[1,:,:]))
    plt.plot(ashp,yres[0:10,:,l].mean(axis=0), label=str(i)+' RI')
    plt.fill_between(ashp,yres1[0:10,:,l].mean(axis=0), yres2[0:10,:,l].mean(axis=0),alpha=.4)
plt.title('Sampling for 10% return value uncertainty')
plt.xlabel('Shape')
plt.ylabel('Number of Samples/RI')
plt.legend()
